...
def integrate_datasets():
    ...
    output_file = f"terminalhill_final_dataset_{timestamp}.txt"
    
    with open(output_file, 'w', encoding='utf-8') as f:
        # ヘッダー情報
        f.write("=" * 80 + "\n")
        f.write("TerminalHill.jugem.jp 最終統合データセット\n")
        ...
        f.write("データ用途: 東京大学修士論文研究（古事記・スピリチュアル分析）\n")
        f.write("対象期間: 2007-2025年（18年間）\n")
        f.write("=" * 80 + "\n\n")
...
if __name__ == "__main__":
    print("🚀 TerminalHill 最終データセット統合")
    print("=" * 60)
    
    result = integrate_datasets()
    if result[0]:
        output_file, article_count, total_chars = result
        print(f"\n🎯 東京大学修士論文研究用データセット完成!")
        print(f"📁 {output_file}")
        print(f"📊 {article_count:,}記事 / {total_chars:,}文字")
        print(f"🔬 研究対象: 古事記・神話・スピリチュアル（2007-2025年）")
        print(f"📈 論文分析可能: テキストマイニング、時系列分析、語彙分析")
    else:
        print("\n❌ データセット統合に失敗しました") 