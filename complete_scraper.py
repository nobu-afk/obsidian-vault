...
"""
完全記事抽出スクリプト
包括的分析で得られた全5,652記事URLを使用した完全抽出
"""
...
def load_article_urls(filename):
    """包括的分析結果ファイルから記事URLを読み込み"""
    urls = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            
        # "全記事URL一覧:" の後の行から記事URLを抽出
        ...
        for line in lines:
            ...
            if "全記事URL一覧:" in line:
                in_url_section = True
                continue
            elif in_url_section and line.startswith("http"):
                urls.append(line)
            elif in_url_section and line.startswith("アーカイブページ一覧:"):
                break
...
def complete_scrape(url_list_file, output_file, delay=2, start_from=0):
    """完全な記事抽出を実行"""
    
    # 記事URLリストを読み込み
    urls = load_article_urls(url_list_file)
    if not urls:
    ...
    print(f"🚀 完全記事抽出開始: {total_urls}記事")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for article in extracted_articles:
            f.write("=" * 80 + "\n")
            f.write(f"記事 {article['index']}/{total_urls + start_from}: {article['title']}\n")
    ...
    print(f"   実行時間: {elapsed_total/3600:.1f}時間")

def main():
    parser = argparse.ArgumentParser(description='完全記事抽出スクリプト')
    ...
    complete_scrape(args.url_list, args.output, args.delay, args.start_from)
