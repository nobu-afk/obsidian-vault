...
| 層 | 名称 | 配置 |
|---|---|---|
| **存在層**（アイデンティティ） | **組織の引力設計者** | profile / achievement / top / WP / whitepaper_optin / seminar-acting |
...
- **THEORY セクション内**：学術名・被引用数・論文タイトル・複数領域統合型のスタンス宣言（学術濃度を意図して残す層）
...
- **正式なフレーム名（経営者向けに翻訳済）**：`強み × 想い × 偏愛` `組織の引力 4 型（整合 / 拡散 / 渇望 / 不毛）` `4 マネジメント（数字 / 業務 / ピープル / 引力相性）`

#### NG / OK 例

```
❌ NG：継続改善は Orbit（月 5 万・最低 1 ヶ月 + 月次更新）で実施（継続率 70% 目標・R の 50% より高い）。
✅ OK：継続改善は Orbit（月 5 万・**最低 1 ヶ月 + 月次更新**・260515 v3.2）で実施。Cultivate 卒業後も多くの経営者が Orbit で伴走を継続されます。
...
### CODE LP 料金記述例外（260516 確立・lint [13]）

- 260515 8 ページピボット原則「公開 8 LP HTML 内の料金記述 0 件」に対して、**CODE LP（`Gravity/Code/LP/index.html`）のみ例外**
- 理由：CODE は税別 5,000 円のアンカリング表記が必須（経営者の認知価格訴求・260516 リブランド v2）
- 実装：`06_開発/scripts/lint/lint_consistency.sh` [13] の PUBLIC_8_LPS ループ内で CODE LP のみ skip + 「例外として許容」メッセージ出力
- 他 7 LP は完全非公開チェック継続

### コーポレート h1 2 行構造化パターン（260516 確立）

- top_本番 h1：`<br>` 1 つで 2 行固定（句読点で改行）
- 例：「優秀人材が集まり、<br>躍動する会社をつくる」
- 16 字（7+9）でリズム良く・PC / モバイル両方で 2 行表示維持
- 反対パターン（避けるべき）：span block + white-space:nowrap + clamp フォントサイズ → モバイル表示崩れる（CODE LP で一時実装したが撤回・自然折り返しに戻した）

### フッター target_blank 削除パターン（260516 確立・site-chrome.js）

- 旧：site-chrome.js b-footer-col 内 全リンクが target="_blank"（別タブ）
- 新：フッターのみ同タブ遷移（HEADER + OVERLAY のサービスリンク target_blank は温存）
- 理由：フッターから別タブが開くと UX 混乱
- 影響範囲：個人軸 2（CODE / Coaching）+ 組織軸 2（組織の引力設計プログラム / 無料 Web 診断）= 4 件削除
