...
# ----------------------------------------------------------------------
# [1.7] 月額表記必須語チェック（v5.5・260515 Cultivate v3.0 + R 戦略転換反映）
# v5.5 確定価格マップ:
#   - Recruit (集まる):     月 35 万・最低 3 ヶ月 + 月次更新（260515 v2.1.4・採用業務継続中継続課金）
#   - Cultivate (躍動する): 月 50 万・最低 6 ヶ月コミット（260515 v3.0・3 設計：ドラムビート/エフィカシー/レコグニション）
#   - Orbit (留まる):       月 5 万・最低 6 ヶ月 + 5 年継続装置（OT-1〜OT-6・260514 v3.0 で月 15 万 → 月 5 万）
#   - Shift (R+C 複合):     月 85 万・段階移行型（R 3 ヶ月 → C 6 ヶ月）
# Coaching = 38 万 6 ヶ月一括維持（月額化しない）
# 検出方針：各 LP に v5.0 月額表記が含まれているかを必須語的にチェック
# ----------------------------------------------------------------------
# [1.7b] Cultivate v2.0 廃止用語チェック（260515 v3.0 反映）
# v3.0 で廃止された v2.0 用語が LP / SSOT / 09_会社OS に残存していないか機械検出
# ----------------------------------------------------------------------
echo ""
echo "[1.7b] Cultivate v2.0 廃止用語チェック（260515 v3.0 反映）"
v3_obsolete_total=0
for term in "3 磁場" "学習磁場" "挑戦磁場" "結束磁場" "CT-1 学習" "CT-2 挑戦" "CT-3 結束"; do
  hits=$(grep -rn "$term" "$ROOT/05_プロダクト" "$ROOT/09_会社OS" 2>/dev/null \
    --include="*.html" --include="*.md" --include="*.sh" \
    | grep -v "_history/" | grep -v "_archive/" | grep -v "260515_GravityCultivate_標準サービス設計_v3.0.md" \
    | grep -v "lint_consistency.sh" | wc -l)
  if [ "$hits" -gt 0 ]; then
    printf "${YEL}⚠ Cultivate v2.0 廃止用語残存「%s」: %s 件${NC}（v3.0 でドラムビート/エフィカシー/レコグニション設計に置換）\n" "$term" "$hits"
    WARNINGS=$((WARNINGS + 1))
    v3_obsolete_total=$((v3_obsolete_total + hits))
  fi
done
[ "$v3_obsolete_total" -eq 0 ] && echo -e "${GRN}✓${NC} Cultivate v3.0 廃止用語チェック PASS（v2.0 残存ゼロ）"
...
# 公開 8 ページ：個人軸 2 LP のみチェック（CODE / Coaching）
check_required "Gravity/Code/LP/index.html" "引力の参謀"
check_required "Gravity/Coaching/LP/index.html" "心の参謀"
# 旧 LP（Recruit/Cultivate/Orbit/Shift/Scan）は 301 リダイレクト稼働中（物理残存）・公開 8 ページから外れたためチェック対象外
# 260515 8 ページピボット前の参謀名チェック履歴：
#   - Gravity/Scan/LP/index.html「引力の参謀（組織軸）」
#   - Gravity/Orbit/LP/index.html「共鳴の参謀」
#   - Gravity/Recruit/LP/index.html「引力の参謀（採用）」
#   - Gravity/Cultivate/LP/index.html「変革の参謀（躍動）」
...
# ----------------------------------------------------------------------
# [12] 8 ページピボット URL チェック（260515 確定・公開 8 LP HTML 内の旧 URL 残存検出）
# 検査対象：公開 8 ページの正本 HTML のみ
#   - /gravity/ : Gravity/_ブランド/LP/index.html
#   - /gravity/code/ : Gravity/Code/LP/index.html
#   - /gravity/coaching/ : Gravity/Coaching/LP/index.html
#   - /gravity/diagnose/ : Gravity/Scan/web-diagnose_本番/index.html
#   - / /profile/ /achievement/ /knowledge/ /news/ : コーポレート/{top,profile,achievement,knowledge,news}_本番/index.html
# 旧 LP（Recruit/Cultivate/Orbit/Shift/Scan の本番 LP）は 301 リダイレクト稼働中・物理残存のため検査対象外
# ----------------------------------------------------------------------
echo ""
...
PUBLIC_8_LPS=(
  ...
  "Gravity/Scan/web-diagnose_本番/index.html"
...
old_url_hits_total=0
for lp in "${PUBLIC_8_LPS[@]}"; do
  f="$ROOT/$lp"
  [ -f "$f" ] || continue
  hits=$(grep -cE "$OLD_URL_PATTERN" "$f" 2>/dev/null)
  hits=${hits:-0}
  if [ "$hits" -gt 0 ]; then
    printf "${YEL}⚠ %s 内に旧 LP URL：%s 件${NC}\n" "$lp" "$hits"
    old_url_hits_total=$((old_url_hits_total + hits))
  fi
done

if [ "$old_url_hits_total" -gt 0 ]; then
  printf "${YEL}⚠ 公開 8 LP 合計 旧 URL 残存：%s 件${NC}（8 ページ構造へ更新要）\n" "$old_url_hits_total"
  WARNINGS=$((WARNINGS + 1))
else
  printf "${GRN}✓${NC} 公開 8 LP 内 旧 URL 残存なし（8 ページピボット URL チェック PASS）\n"
fi
...
for lp in "${PUBLIC_8_LPS[@]}"; do
  ...
  hits=${hits:-0}
  if [ "$hits" -gt 0 ]; then
    printf "${RED}✗ %s 内に料金記述：%s 件${NC}\n" "$lp" "$hits"
    price_hits_total=$((price_hits_total + hits))
  fi
done

if [ "$price_hits_total" -gt 0 ]; then
  printf "${YEL}⚠ 公開 8 LP 合計 料金記述：%s 件${NC}（260515 堀田流・完全非公開化要・段階 2-3 残課題）\n" "$price_hits_total"
  WARNINGS=$((WARNINGS + 1))
else
  printf "${GRN}✓${NC} 公開 8 LP 料金完全非公開チェック PASS\n"
fi
