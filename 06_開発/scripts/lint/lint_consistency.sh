...
# ----------------------------------------------------------------------
# [1.7] 月額表記移行期警告（R5・260513 Phase C 22-CS 追加・Phase B 22-CI 解消予定）
# R5 確定価格マップ（月額メイン + 総額括弧併記）:
#   - Recruit:    月 26.7 万円（3 ヶ月総額 80 万）
#   - Cultivate:  月 25 万円（6 ヶ月総額 150 万）
#   - Shift Full: 月 24.4 万円（9 ヶ月総額 220 万）
# Coaching = 38 万 6 ヶ月一括維持（月額化しない）／ Orbit = 既月 15 万表記済 → 対象外
# 検出方針：各 LP に「月 2X 万」表記（月額メイン）が含まれているかを必須語的にチェック
# ----------------------------------------------------------------------
echo ""
echo "[1.7] 月額表記移行期警告（R5・Phase B 22-CI で解消予定）"
...
check_monthly_pricing() {
  ...
  if [ "$monthly_count" -eq 0 ]; then
    printf "${YEL}⚠ %s 月額表記なし${NC}（R5 SSOT: 月額メイン + 総額括弧併記 / Phase B 22-CI 解消予定）\n" "$label"
    WARNINGS=$((WARNINGS + 1))
  else
...
check_monthly_pricing "Gravity/Recruit/LP/index.html" "Recruit（月 26.7 万 / 3 ヶ月総額 80 万）" "月 ?26\.7 *万|月 ?27 *万"
check_monthly_pricing "Gravity/Cultivate/LP/index.html" "Cultivate（月 25 万 / 6 ヶ月総額 150 万）" "月 ?25 *万"
check_monthly_pricing "Gravity/Shift/LP/index.html" "Shift Full（月 24.4 万 / 9 ヶ月総額 220 万）" "月 ?24\.4 *万|月 ?24 *万"
