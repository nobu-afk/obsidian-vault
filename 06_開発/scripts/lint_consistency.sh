...
check_forbidden "Shift 60万" "旧Shift価格" "Shift R/A 各 80 万"
...
# 260503 二層命名運用（B 案）：対外 HTML（05_プロダクト/ 配下）で内的呼称が残っていたら警告
# - LP / WP / コーポレートでは外的呼称「Gravity Recruit / Gravity Activate / Gravity Shift」が正
# - 「Shift R」「Shift A」「Shift Full」「Gravity Shift R」「Gravity Shift A」は対外文脈で禁則
...
check_forbidden_warning "Gravity Shift A" "260503 二層命名運用（対外）" "Gravity Activate"
check_forbidden_warning "Shift R" "260503 二層命名運用（対外・社内呼称）" "Gravity Recruit"
check_forbidden_warning "Shift A" "260503 二層命名運用（対外・社内呼称）" "Gravity Activate"
check_forbidden_warning "Shift Full" "260503 二層命名運用（対外・社内呼称）" "Gravity Shift（R+A 複合）"
check_forbidden_warning "/gravity-shift-a/" "260503 β 並列型 URL 構造" "/gravity-activate/"
...
# 260503 β 並列型・新設 LP（Gravity Recruit / Gravity Activate）
check_required "GravityRecruit/LP/index.html" "変革の参謀"
check_required "GravityActivate/LP/index.html" "変革の参謀"
...
top_link_missing=0
for lp in GravityCode GravityScan GravityRecruit GravityActivate; do
...
[ "$top_link_missing" -eq 0 ] && echo -e "${GRN}✓${NC} 全 LP（CODE/Scan/Recruit/Activate）に TOP 導線あり ／ GravityShift/Orbit/Coaching は minimal LP 運用のため対象外"
