...
const QUESTIONS = [
  ...
  // ============================
  // Q2: 動詞仮説（ストーリー・4択・voice有）
  // ============================
  {
    text: '事業が軌道に乗り始めた時、あなたが最もエネルギーを注ぐのは何か？',
    subtext: '',
    type: 'choice',
    category: 'verb',
    options: [
      { text: '新しい構造を設計する', voice: 'まだ見えていないものを形にしたい' },
      { text: '相手の本音を言語化する', voice: '人の中にある未言語化を翻訳したい' },
      { text: '自分で現場を動かす', voice: '自分の手で事業を動かす実感がほしい' },
      { text: '仕組みを作って任せる', voice: '自分がいなくても回る状態を作りたい' },
    ]
  },

  // ============================
  // Q3: 環境発火（ストーリー・4択・voice有）
  // ============================
  {
    text: '月曜朝、今週の予定を見る。\nエネルギーが最も湧く予定はどれ？',
    subtext: '',
    ...
    category: 'env-fire',
    options: [
      { text: '1対1の深い対話', voice: '人の未言語化に触れる時間' },
      { text: '構造が見えない混沌との格闘', voice: '整理されていない場にこそ動ける' },
      { text: '新しい領域の学び・実験', voice: '未知への好奇心' },
      { text: '数値と向き合う戦略立案', voice: '構造を数字で読み解く' },
    ]
  },

  // ============================
  // Q4: 環境停止（ストーリー・4択・voice有）
  // ============================
  {
    text: '逆に、エネルギーが最も削られる予定はどれ？',
    subtext: '',
    type: 'choice',
    category: 'env-stop',
    options: [
      { text: '同じことの繰り返しの定例', voice: '変化がない時間の苦痛' },
      { text: '自分が本気でないセールス活動', voice: '売り込むことへの違和感' },
      { text: '決まりきった運用業務', voice: '完成したものを回す退屈' },
      { text: '数字の報告と説明責任', voice: '評価されるために成果を見せる窮屈さ' },
    ]
  },
...
const App = {
  ...
  render() {
    ...
    if (q.type === 'choice') {
      ...
      observations.innerHTML = '<p style="font-size:13px;color:#0f172a;line-height:1.8;margin:0 0 10px;font-weight:700;">AI が対話から抽出する 5 つの観点：</p>'
        ...
        + '<li><strong>才能</strong> ── 自然にできてしまう動きと、それが発火する環境</li>'
      ...
      const checkLabels = [
        ...
        '才能 ── 自然にできてしまう動きと発火する環境が対話に含まれている',
  ...
  showSessionPreview() {
    ...
    if (summaryList) {
      ...
      const labels = ['立場', '動詞の仮説', 'エネルギーが湧く場', '削られる場', '言行ギャップ'];
      for (let i = 0; i < 5; i++) {
        const q = QUESTIONS[i];
