...
0. **★ NEW（v1.1）「組織の引力とは何か」定義ボックス（gravity-definition-box）** ← core-quote の前、Block A の最初に配置
   - 3-5 行で組織の引力定義を開示
   - 文言例：「組織の引力＝人が離れない・自発的に集まり躍動する『場の力』。経営者個人の引力（CODE = Why × 才能 × 偏愛）が、組織への翻訳を経て立ち上がる。集まる軸（採用パイプ・最終辞退率・自覚度）× 躍動軸（定着率・幹部発話・エンゲージメント・離脱予兆）の 2 マトリクスで 4 型に分類される。事業の天井は、組織の引力の天井である。このレポートは、あなたの組織の引力タイプを判定し、Pre-Shift 適合パッケージ（Gravity Recruit／Cultivate／Shift）を示す。」
   - ★ 経営者が「拡散型」「集まる軸」「躍動軸」を理解しないままレポートを読み進める構造を防ぐ

1. **★一撃1：組織の引力タイプ確定の宣言（core-quote）** ← gravity-definition-box の直後
...
### gravity-definition-box（★ NEW v1.1・Block A 最初に必ず配置）
<div class="gravity-definition-box">
  <h4>組織の引力とは</h4>
  <p>組織の引力＝人が離れない・自発的に集まり躍動する「場の力」。経営者個人の引力（CODE = Why × 才能 × 偏愛）が、組織への翻訳を経て立ち上がる。</p>
  <p><strong>集まる軸</strong>（採用パイプ・最終辞退率・自覚度）× <strong>躍動軸</strong>（定着率・幹部発話・エンゲージメント・離脱予兆）の 2 マトリクスで 4 型に分類される。</p>
  <p><strong>事業の天井は、組織の引力の天井である。</strong>このレポートは、あなたの組織の引力タイプを判定し、Pre-Shift 適合パッケージ（Gravity Recruit／Cultivate／Shift）を示す。</p>
</div>

### core-quote（タイプ確定の宣言・★Block A・gravity-definition-box の直後・analyst-intro は書かない★）
...
生成ルール（260508 v1.1 改修・公開向け表記厳守 + ページ最適化）：
- **3 ブロック構造**：Block A（4 型判定 + 「組織の引力とは」定義開示）→ Block B（7 項目スコアリング）→ Block C（Pre-Shift 適合診断＋ロードマップ）
- **★ Block A 冒頭に「組織の引力とは」定義ボックス（gravity-definition-box）を必ず配置**：core-quote の前に配置・3-5 行で組織の引力定義を開示・経営者が前提理解なくレポートを読む構造を防ぐ
...
生成ルール（260508 v1.1 改修・公開向け表記厳守 + ページ最適化）：
- 3 ブロック構造（Block A：4 型判定 + 「組織の引力とは」定義開示／Block B：7 項目スコアリング／Block C：Pre-Shift 適合診断＋ロードマップ）
- ★ Block A 冒頭に「組織の引力とは」定義ボックス（gravity-definition-box）を必ず配置・3-5 行で開示
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* v1.4（260508）：Block A 1P 集約のため type-matrix-box と gravity-gap-box にスコープ限定でピンポイント圧縮（他セクションには影響させない） */
  .type-matrix-box { padding: 14px 20px !important; margin: 12px 0 !important; }
  .type-matrix-box h4 { font-size: 10pt !important; margin: 0 0 8px !important; }
  .type-matrix-box .gi-summary { font-size: 10.5pt !important; padding: 8px 12px !important; margin: 0 0 10px !important; line-height: 1.5 !important; }
  ...
  .type-matrix-box .gi-table th { padding: 8px 10px !important; font-size: 9pt !important; }
  .type-matrix-box .gi-table td { padding: 8px 10px !important; font-size: 9pt !important; line-height: 1.5 !important; }
  .gravity-gap-box { padding: 14px 20px !important; margin: 12px 0 !important; }
  .gravity-gap-box h4 { font-size: 10pt !important; margin: 0 0 10px !important; }
  .gravity-gap-box > p { padding: 6px 10px !important; margin: 0 0 6px !important; font-size: 9pt !important; line-height: 1.5 !important; }
  /* issue 優先順位付き */
