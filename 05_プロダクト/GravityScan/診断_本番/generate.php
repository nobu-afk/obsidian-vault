...
- **同質性バブル警鐘：** 集まる軸 ◎ 確定後でも、Page 2017 Diversity Bonus に基づき「Fit ◎ × 多様性 △ = 拡散型予備軍」として Block C analyst-note で警鐘を鳴らす
...
### 4 象限判定マトリクス（260510 v0.4・新規）

| 象限 | 不確実性耐性 | 曖昧さ耐性 | 経営者像 | 推奨パッケージ |
|---|---|---|---|---|
| **A：完全覚悟型** | 高 | 高 | 結論先送り OK・複数解釈保持 OK | **Gravity Cultivate 単独 80 万**（Coaching 並走不要）|
| **B：結論先送り型** | 低 | 高 | 結論を出すのは焦るが多解釈は持てる | **Cultivate + Coaching 並走 128 万**（Coaching で結論力育成）|
| **C：単線思考型** | 高 | 低 | 待てるが「正解は 1 つ」志向 | **Cultivate + Coaching 並走 128 万**（Coaching で多解釈耐性育成）|
| **D：完全 have to 型** | 低 | 低 | 「先回りして答えを準備する」型 | **Gravity Coaching 単独先行 38 万・6 ヶ月**（覚悟先決）|

### 判定別 Block C 内容調整（260510 v0.4）

- **A 完全覚悟型：** Cultivate 単独推奨 / analyst-note は前向きな後押し
- **B 結論先送り型：** Cultivate + Coaching 並走 128 万 / Coaching の **「結論を出す覚悟を育てる」**側面を明示
- **C 単線思考型：** Cultivate + Coaching 並走 128 万 / Coaching の **「多解釈に耐える覚悟を育てる」**側面を明示
- **D 完全 have to 型：** Coaching 単独先行 / final-question で「答えのない 3 ヶ月を待てるか」を強く問い返す
...
#### Block C：必ず覚悟確認 4 象限ラベルを出力
- ❌「中判定」「高判定」「低判定」だけの単軸表記禁止
- ✅ **必ず以下 4 象限のいずれか**で判定ラベルを明示：
  - **A：完全覚悟型**（不確実性高 × 曖昧さ高）
  - **B：結論先送り型**（不確実性低 × 曖昧さ高）
  - **C：単線思考型**（不確実性高 × 曖昧さ低）
  - **D：完全 have to 型**（不確実性低 × 曖昧さ低）
- 経営者の発話パターンから 2 軸スコア（高/低）を判定 → 4 象限のラベル + 推奨パッケージカスタマイズを Block C で出力

#### Block C：拡散型 / 整合型なら同質性バブル警鐘を必ず出力
- 集まる軸 ◎ 確定後、analyst-note 内で「もう 1 つ正直に言う：集まる軸 ◎ は健全だが、Fit が高すぎる組織は同質性バブルに陥るリスクがある（Page 2017）...」を必ず 1 段落出力
...
1. **★ Pre-Shift 適合診断（shift-fit-box--primary・最優先・厚い）**
   - **推奨パッケージ：** [Gravity Recruit / Gravity Cultivate / **Gravity Cultivate + Coaching 並走パッケージ 128 万**（260501 新規・**260510 v0.4 で覚悟確認 B 結論先送り型 / C 単線思考型**の標準・拡散型限定）/ Gravity Recruit + Gravity Cultivate 順次受講（不毛型・商談時に Gravity Shift 150 万案内）/ Gravity Orbit 直接 / Gravity Coaching 先行（**260510 v0.4 で覚悟確認 D 完全 have to 型**標準）のいずれか]
   - **推奨理由：** 4 型判定 + 引力欠損ポイント + 経営者の覚悟確認（トランスクリプトから）+ **覚悟確認 4 象限ラベル**（260510 v0.4 で 2 軸 4 象限化）+ **採用コスト対効果**（採用費／昇給原資の倍率・260501 夕 追加・Gravity Recruit 推奨時のみ）の 5 軸で 3-5 行・**★ 必ず A/B/C/D 象限ラベルで明示すること**
   - **価格・期間：** Gravity Recruit 80 万・3 ヶ月 ／ Gravity Cultivate 80 万・3 ヶ月 ／ **★ Gravity Cultivate + Coaching 並走パッケージ 128 万・6 ヶ月**（Scan 10 + Cultivate 80 + Coaching 38・**覚悟確認 B 型 / C 型**の標準・拡散型限定）／ Gravity Shift 150 万・6 ヶ月（R+C 複合・10 万割引）／ Gravity Orbit 月 15 万 ／ Gravity Coaching 38 万・6 ヶ月（**覚悟確認 D 型**標準）
...
| **【260510 新規】PO Fit 認識** | **R-α' 採用ペルソナの価値観適合再設計**：求人原稿・スカウト文面の Why 起点書き直しに加え、**価値観一致度を内定者・候補者へのサーベイで定量化**（PO Fit 認識スコア化）|
| 優秀人材定着率（フェーズ移行型・入社後 6 ヶ月以内）| **R-γ 内定〜入社 6 ヶ月の離脱予防設計**：内定承諾後 30 分の経営者対話／入社初日チェックリスト／1-3-6 ヶ月の離脱ジャーニー点検／**入社後 3 ヶ月時点で Job Crafting セッション**（Wrzesniewski 2013・Calling 型移行支援）|
| 優秀人材定着率（入社 1 年以降の離脱）| **C-α 評価 × 1on1 再設計**：個人の偏愛と組織目標を接続する評価基準と 1on1 のテーマ設計を再構築・**Introjected Regulation スコア化**（Van den Broeck 2017）+ **基本心理欲求 3 軸**（自律性・有能感・関係性・Vansteenkiste & Ryan 2013）+ **給与制度 × 動機 × 業績の整合チェック**（Gagne 2014）|
| **【260510 改訂】心理的安全性 4 行動**（失敗共有 / 質問 / 助け求め / 実験試行）| **C-δ 幹部マネジメント育成 × 自律発議の場作り**：幹部が自分の言葉で組織の方向性を語り、自律発議できる土台を立ち上げる ＋ **月次サーベイで 4 行動の頻度測定**（Edmondson 1999）+ **謙虚なリーダーシップ 3 次元育成**（Owens & Hekman 2018）|
...
3. **★一撃3：最大リスク（analyst-note内・石井一人称・260510 v0.4 で同質性バブル警鐘追加）**
   ...
   - **★ 同質性バブル警鐘（260510 v0.4 新規・拡散型 / 整合型のみ追加）：** 集まる軸 ◎ × 躍動軸 △/◎ の組織には、Page 2017（Diversity Bonus）に基づく以下の警鐘を 1 段落追加
     > 「もう 1 つ正直に言う。集まる軸 ◎ は健全だが、**Fit が高すぎる組織は同質性バブルに陥るリスク**がある。能力の高い個人を集めるより『異なるツールボックスを持つ人々』が複雑問題を解決する（Page 2017）。Recruit / Cultivate 実装時は **多様性 vs Fit** の二軸で採用基準を再点検すべきだ。」
   - 渇望型・不毛型では出力しない（集まる軸 △ のため適用外）
...
5. **path-cards（4 型判定推奨ロジックに従う・260505 Cultivate 化反映・公開向け URL）**
   ...
   - **★ URL 振り分けルール（260505 Cultivate 化整合）：**
     ...
     - Gravity Cultivate + Coaching 並走 128 万推奨（**覚悟確認 B 型 / C 型**・拡散型限定）→ `https://growthfix.jp/gravity-cultivate/#pricing`
     - Gravity Orbit 直接（整合型）→ `https://growthfix.jp/gravity-orbit/`
     - Gravity Coaching 先行（**覚悟確認 D 完全 have to 型**）→ `https://growthfix.jp/gravity-coaching/`
...
<div class="shift-fit-box shift-fit-box--primary">
  ...
  <p><strong>価格・期間：</strong>[Gravity Recruit 80 万・3 ヶ月／Gravity Cultivate 80 万・3 ヶ月／Gravity Cultivate + Coaching 並走パッケージ 128 万・6 ヶ月（拡散型・**覚悟確認 B 型 / C 型**限定）／Gravity Shift 150 万・6 ヶ月（R+C 複合・10 万割引）／Gravity Orbit 月 15 万／Gravity Coaching 38 万・6 ヶ月（**覚悟確認 D 型**標準）]</p>
  <p class="covenant-quadrant"><strong>★ 覚悟確認 4 象限判定（必須出力）：</strong>このレポートでは経営者の覚悟確認を **必ず A/B/C/D の 4 象限ラベル**で明示します。**[判定：A 完全覚悟型 ／ B 結論先送り型 ／ C 単線思考型 ／ D 完全 have to 型 のいずれか]**</p>
</div>
...
<div class="analyst-note">
  ...
  <!-- 同質性バブル警鐘・260510 v0.4 新規・拡散型 / 整合型のみ表示 -->
  <p><strong>もう 1 つ正直に言う：</strong>集まる軸 ◎ は健全だが、**Fit が高すぎる組織は同質性バブルに陥るリスク**がある。能力の高い個人を集めるより「異なるツールボックスを持つ人々」が複雑問題を解決する（Page 2017）。Recruit / Cultivate 実装時は **多様性 vs Fit** の二軸で採用基準を再点検すべきだ。</p>
</div>
